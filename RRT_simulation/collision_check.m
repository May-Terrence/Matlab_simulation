...
for i = 1:length(map.obs_polygon_boom)
    obs_polygon_i = map.obs_polygon_boom{1,i};
    in = inpolygon(x_new(1),x_new(2),obs_polygon_i{1,1}(:,1),obs_polygon_i{1,1}(:,2));
    if in == 1
        is_in = 1;
        break
...
for i = 1:size(map.obs_circle_boom,1)
    t = linspace(0, 2*pi);
...
if is_in == 0
    ...
    polygon_num = length(map.obs_polygon_boom);
    for k = 1:polygon_num
        vertex_num = size(map.obs_polygon_boom{1,k}{1,1},1);
        for m = 1:vertex_num
            if m == vertex_num
                A = map.obs_polygon_boom{1,k}{1,1}(m,:);
                B = map.obs_polygon_boom{1,k}{1,1}(1,:);
            else
                A = map.obs_polygon_boom{1,k}{1,1}(m,:);
                B = map.obs_polygon_boom{1,k}{1,1}(m+1,:);
            end
            is_intersect = intersect_check(A,B,x_nearest,x_new);
            if is_intersect == 1
                is_collision = true;
    ...
    for k = 1:size(map.obs_circle_boom,1)
        lambda = dot(map.obs_circle_boom(k,1:2)-x_nearest,x_new-x_nearest) / (norm(x_new-x_nearest)*norm(x_new-x_nearest));
        if lambda < 1 && lambda > 0
            g = x_nearest + lambda*(x_new-x_nearest);
            d = norm(g-map.obs_circle_boom(k,1:2));
            if d < map.obs_circle_boom(k,3)
                is_collision = true;
            end
        end
    end
