...
h = legend('\fontname{宋体}起点', '\fontname{宋体}目标点','\fontname{宋体}障碍物', ...
    '\fontname{宋体}障碍物膨胀边缘', '\fontname{宋体}初始折线路径', ...
...
sub_axes = axes('Position', [0.44, 0.2, 0.2, 0.2]); % [左, 下, 宽, 高]
% 画多边形障碍物
for i = 1:size(map.obs_polygon_boom,2)
    % per obs
    vertexs = map.obs_polygon_boom{i}{1};
...
annotation('textarrow', [0.408, 0.448], [0.40, 0.38], 'FontSize', 10);
...
% fimename = 'D:\Flycontrol\SCUT_thesis\Fig\chapter5\CRRT_Star算法在高密度障碍物地图中的路径规划结果.pdf';
% PlotToFileColorPDF(RRTC,fimename,15,7.45);
% fimename = 'D:\Flycontrol\SCUT_thesis\Fig\chapter5\B样条曲线对折线路径的平滑结果.pdf';
% PlotToFileColorPDF(RRTC,fimename,15,7.45);